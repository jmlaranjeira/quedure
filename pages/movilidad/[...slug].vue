...
const { data: article } = await useAsyncData(`movilidad-article-${route.path}`, () => {
  return queryCollection('movilidad').path(route.path).first()
})

// If no article, get articles in this subcategory
const { data: subcategoryArticles } = await useAsyncData(`movilidad-list-${route.path}`, async () => {
  if (article.value) return null
  const all = await queryCollection('movilidad').order('updatedAt', 'DESC').all()
    ...
    <section class="bg-gradient-to-br from-primary-600 to-primary-800 text-white py-16">
      <div class="container-wide">
        <nav class="text-sm mb-4">
          ...
          <NuxtLink to="/movilidad" class="text-primary-200 hover:text-white">Movilidad</NuxtLink>
          <span class="mx-2 text-primary-300">/</span>
          <span>{{ subcategoryName }}</span>
        </nav>
        <h1 class="text-4xl font-bold mb-4">{{ subcategoryName }}</h1>
        <p class="text-xl text-primary-100 max-w-2xl">
          Guías y recomendaciones de {{ subcategoryName?.toLowerCase() }} duraderos y sostenibles.
        </p>
      </div>
    </section>
