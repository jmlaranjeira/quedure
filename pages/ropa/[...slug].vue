...
const { data: article } = await useAsyncData(`ropa-article-${route.path}`, () => {
  return queryCollection('ropa').path(route.path).first()
})

// If no article, get articles in this subcategory
const { data: subcategoryArticles } = await useAsyncData(`ropa-list-${route.path}`, async () => {
  if (article.value) return null
  const all = await queryCollection('ropa').order('updatedAt', 'DESC').all()
...
if (article.value) {
  ...
  useSeo({
    title: `${subcategoryName.value} - Ropa | QueDure.es`,
    ...
    <section class="bg-gradient-to-br from-primary-600 to-primary-800 text-white py-16">
      <div class="container-wide">
        <nav class="text-sm mb-4">
          ...
          <NuxtLink to="/ropa" class="text-primary-200 hover:text-white">Ropa</NuxtLink>
          <span class="mx-2 text-primary-300">/</span>
          <span>{{ subcategoryName }}</span>
        </nav>
        <h1 class="text-4xl font-bold mb-4">{{ subcategoryName }}</h1>
        <p class="text-xl text-primary-100 max-w-2xl">
          Guías y recomendaciones de {{ subcategoryName?.toLowerCase() }} duraderos y éticos.
        </p>
      </div>
    </section>
